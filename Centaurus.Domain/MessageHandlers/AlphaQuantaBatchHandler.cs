...
namespace Centaurus.Domain
{
    public class AlphaQuantaBatchHandler : MessageHandlerBase<OutgoingConnection>
    {
        ...
        public override string SupportedMessageType { get; } = typeof(AlphaQuantaBatch).Name;
        ...
        private async Task AddQuantaToQueue(OutgoingConnection connection, IncomingMessage message)
        {
            ...
            var quantaBatch = (AlphaQuantaBatch)message.Envelope.Message;
            ...
            {
                var quantum = (Quantum)processedQuantum.Qunatum;
                //get last known apex
                var lastKnownApex = quantumHandler.LastAddedQuantumApex;
                if (quantum.Apex <= lastKnownApex)
                {
                    Console.WriteLine($"Received {quantum.Apex}, expected {lastKnownApex + 1}");
                    continue;
                }
