...
namespace Centaurus.DAL
{
    ...
    {
        public static BsonObjectId EncodeId(long apex, int index)
        {
            var rawId = new byte[12];
            BinaryPrimitives.WriteInt64BigEndian(rawId.AsSpan(0, 8), apex);
            BinaryPrimitives.WriteInt32BigEndian(rawId.AsSpan(8, 4), index);
            return new BsonObjectId(new ObjectId(rawId));
        }

        public static (long apex, int index) DecodeId(byte[] effectId)
        {
            ...
            return (
                ...
                index: BinaryPrimitives.ReadInt32BigEndian(effectId.AsSpan(8, 4))
            );
        }

        public static (long apex, int index) DecodeId(BsonObjectId effectId)
        {
            if (effectId == null)
